...
from telegram.ext import (Updater, CommandHandler, MessageHadler, Filters, ConversationHandler)
...
def start(update, _):
    update.message.reply_text(f'Привет, {update.effective_user.first_name}, я - калькулятор. Выберите команду.\n' 'Команда /cansel, чтобы прекратить разговор. \n\n')
    update.message.reply_text('1 - рациональные числа; \n2 - комплексные числа; \n3 - Выйти из калькулятора \n')
    return CHOICE

def choice(update, context):
    user = update.messege.from_user
    logger.info("Выбор операции: %s: %s", user_first_name, updater.messege.text)
    ...
    if user_choice in '123':
        if user_choice == '1':
            ...
            return RATIONAL_ONE
        if user_choice == '2':
            context.bot.send_messege(update.effective_chat.id, 'Введите Re и Im первого числа через пробел: ')
            return RATIONAL_ONE
        if user_choice == '3':
            update.message.reply_text('Спасибо, до скорых встречь :)')
...
def rational_one(update, context):
    ...
    if get_rational.isdigit():
        ...
        update.message.reply_text('Введите пожалуйста число')
...
def rational_two(update, context):
    ...
    if get_rational.isdigit():
        ...
        update.message.reply_text('Выберите действия: \n\n+ - для сложения; \n- - для вычетания; \n* - для умножения; \n/ - для деления \n')
...
def operations_rational(update, context):
    ...
    rational_one = context.user_data.get('rational_two')
    if user_choice in '+-/*':
        if user_choice == '+':
...
def complex_two(update, context):
    ...
    if ' ' in test and (test.replace(' ', '')).isdigit():
        ...
        update.message.reply_text(f'Второе число {complex_two}, Выберите действия: \n\n+ - для сложения; \n- - для вычетания; \n* - для умножения; \n/ - для деления \n ')
...
def operations_complex(update, context):
    ...
    complex_one = context.user_data.get('complex_two')
    if user_choice in '+-/*':
        if user_choice == '+':
...
def cansel(update, _):
    user = update.message.from_user
    logger.info("Пользователь %s прекратил разговор", user.first_name)
    update.message.reply_text('Спасибо, до новых встречь')
    return ConversationHandler.END 
