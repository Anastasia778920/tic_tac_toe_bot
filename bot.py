...
bot = telebot.TeleBot('5785132933:AAGx15utuajdxDR2tQuEGbjoyDEcJkKr6NM')
...
def start_message(message):
    bot.send_message(message.chat.id, "Привет! Я бот, который умеет и хочет с тобой сыграть в крестики-нолики. Если хочешь сыграть со мной, то просто напиши /start.")


@bot.message_handler(commands=['end'])
def end_game(message):
    bot.send_message(message.chat.id,
                     "Мне очень жаль, ведь мне так нравилось с тобой играть :( Но я все равно жду тебя на новую игру!"
                     "\n P.S.: Чтобы сыграть снова, напиши /start")
...
def player_turn(message):
    ...
    while player_move in all_moves:
        bot.reply_to(message, "Это место уже занято. Выберите другое поле.")
    ...
    for i in win_list:
        if i[0] in players_moves and i[1] in players_moves and i[2] in players_moves:
            bot.send_message(message.chat.id, "Вы победили, поздравляю!")
